...
rtc = machine.RTC()
rtc.datetime((2020, 1, 8, 0, 10, 1, 0, 0)) # fourth is day of week, fifth is hour
now = rtc.datetime()

def rect(rx, ry, wx, wy, cl):
  "Draws a rectangle with color cl"
  for x in range(rx, rx + wx):
    for y in range(ry, ry + wy):
      display.pixel(x, y, cl)

def disp2dig(value, xoff):
  "Displays value at xoff position"
  rect(xoff, 4, 16, 8, 0)
  if( value < 10):
    display.text(str(0), xoff, 4)
    xoff += 8
  display.text(str(value), xoff, 4)
...
while True:
  if(rtc.datetime()[6] != now[6]):
    disp2dig(now[6], 76)
    if(rtc.datetime()[5] != now[5]):
      disp2dig(now[5], 56)
      if(rtc.datetime()[4] != now[4]):
        disp2dig(now[4], 36)
    now = rtc.datetime()
    display.show()
    print("#", end="")
